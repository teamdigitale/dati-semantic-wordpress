...
class OxyToolboxLicense {
	...
	static function activate_license() {
		...
		if( isset( $_POST[self::$prefix.'license_activate'] ) ) {
			...
			if ( ! empty( $message ) ) {
				...
				$redirect = add_query_arg( array( 'sl_activation' => 'false', 'message' => urlencode( $message ), 'nonce'	=> wp_create_nonce( self::$prefix . 'license_nonce' ) ), $base_url );
	...
	static function admin_notices() {
		if ( isset( $_GET['sl_activation'] ) && ! empty( $_GET['message'] ) && isset( $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], self::$prefix . 'license_nonce' ) && current_user_can( 'manage_options' ) ) {
