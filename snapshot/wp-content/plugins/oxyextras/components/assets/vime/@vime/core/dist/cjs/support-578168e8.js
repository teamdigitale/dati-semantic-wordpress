...
const onTouchInputChange = (callback) => {
  if (!IS_CLIENT)
    return () => { };
  let lastTouchTime = 0;
  const offTouchListener = dom.listen(document, 'touchstart', () => {
    lastTouchTime = new Date().getTime();
    callback(true);
  }, true);
  const offMouseListener = dom.listen(document, 'mousemove', () => {
...
const canFullscreenVideo = () => {
  ...
  // @ts-ignore
  return dom.isFunction(video.webkitEnterFullscreen);
};
/**
...
const canUsePiPInSafari = () => {
  ...
  const video = document.createElement('video');
  // @ts-ignore
  return dom.isFunction(video.webkitSupportsPresentationMode)
    // @ts-ignore
    && dom.isFunction(video.webkitSetPresentationMode)
    && !IS_IPHONE;
};
...
exports.canFullscreenVideo = canFullscreenVideo;
exports.canPlayHLSNatively = canPlayHLSNatively;
