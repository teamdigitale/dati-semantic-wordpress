...
function ct_view_meta_box_callback( $post ) {
	...
	if ($screen->post_type != "ct_template") {
		...
		$ct_other_template = oxy_get_post_meta( $post->ID, 'ct_other_template', true );
		// check if the other template contains ct_inner_content
		$shortcodes = false;
		$json = false;
		if($ct_other_template && $ct_other_template > 0) {
			$json = 	  oxy_get_post_meta($ct_other_template, 'ct_builder_json', true);
			$shortcodes = oxy_get_post_meta($ct_other_template, 'ct_builder_shortcodes', true);
		} elseif ( $generic_view && $ct_other_template != -1) {
			$json = 	  oxy_get_post_meta($generic_view->ID, 'ct_builder_json', true);
			$shortcodes = oxy_get_post_meta($generic_view->ID, 'ct_builder_shortcodes', true);
		}
		...
		?>
					...
					<?php
						...
						foreach($templates as $template) {
							...
							if ( $json_to_check ) {
								...
								$codes = '';
								$codes = oxy_get_post_meta($template->id, 'ct_builder_shortcodes', true);
							...
							$has_a_parent = oxy_get_post_meta( $template->id, 'ct_parent_template', false );
			...
			<?php
				...
				$parent_of_template = oxy_get_post_meta( $template_id, 'ct_parent_template', false );
			...
			?>
				...
				<div id="ct-builder-shortcodes" style="display:none">
					<textarea readonly="true" class="widefat" rows="8" name="ct_builder_shortcodes" id ="ct_builder_shortcodes"><?php 
						echo htmlentities( oxy_get_post_meta( $post->ID, 'ct_builder_shortcodes', true ) ); 
					?></textarea>
				</div>
				<p>
				...
				<div id="ct-builder-json" style="display:none">
					<textarea class="widefat" rows="8" name="ct_builder_json" id ="ct_builder_json"><?php 
						echo htmlentities(  oxy_get_post_meta( $post->ID, 'ct_builder_json', true ) ); 
					?></textarea>
				</div>
	...
	// Button only for "Templates"
		...
		$ct_template_type = oxy_get_post_meta( $post->ID, 'ct_template_type', true);

		
			$ct_parent_template = oxy_get_post_meta( $post->ID, 'ct_parent_template', true );
			$shortcodes = '';
			$json = '';
			
			if($ct_parent_template && $ct_parent_template > 0) {
				$shortcodes = oxy_get_post_meta($ct_parent_template, 'ct_builder_shortcodes', true);	
				$json = oxy_get_post_meta($ct_parent_template, 'ct_builder_json', true);
			}
		...
		if($ct_template_type != 'reusable_part') {
					...
					<select name="ct_parent_template" id="ct_parent_template" data-parent-template="<?php echo (!$ct_parent_template || intval($ct_parent_template) === 0)?'0':intval($ct_parent_template);?>">
						...
						foreach($templates as $template) {
							...
							if(!($ct_template_type && $ct_template_type =='reusable_part') && intval($template->id) != $post->ID) {
								...
								$its_parent = oxy_get_post_meta( intval($template->id), 'ct_parent_template', true );
								$is_inherited = false;
								$count = 0; // fail safe
								while($its_parent !== false && $count < 9) {
									...
									$count++;
									$its_parent = oxy_get_post_meta( intval($its_parent), 'ct_parent_template', true );
								}
			...
			<?php
				...
				$template_post_types 	= oxy_get_post_meta( $post->ID, 'ct_template_post_types', true );

				if(!is_array($template_post_types)) {
					$template_post_types = array();
				}

				$template_single_all 	= oxy_get_post_meta( $post->ID, 'ct_template_single_all', true );


				$categories_all 		= sanitize_text_field(oxy_get_post_meta( $post->ID, 'ct_template_categories_all', true ));
				$tags_all 				= sanitize_text_field(oxy_get_post_meta( $post->ID, 'ct_template_tags_all', true ));
				$custom_taxonomies_all 	= sanitize_text_field(oxy_get_post_meta( $post->ID, 'ct_template_custom_taxonomies_all', true ));
				
				$use_taxonomies 		= oxy_get_post_meta( $post->ID, 'ct_use_template_taxonomies', true );
				$template_raw_taxonomies 	= oxy_get_post_meta( $post->ID, 'ct_template_taxonomies', true );
				...
				$template_apply_if_post_of_parents = oxy_get_post_meta( $post->ID, 'ct_template_apply_if_post_of_parents', true );
				$template_post_of_parents 	= oxy_get_post_meta( $post->ID, 'ct_template_post_of_parents', true );
				...
				$template_all_archives = oxy_get_post_meta( $post->ID, 'ct_template_all_archives', true );

				$template_apply_if_archive_among_taxonomies = oxy_get_post_meta( $post->ID, 'ct_template_apply_if_archive_among_taxonomies', true );
				$template_archive_among_taxonomies 	= oxy_get_post_meta( $post->ID, 'ct_template_archive_among_taxonomies', true );
				if ( !$template_archive_among_taxonomies ) {
					$template_archive_among_taxonomies = array();
				}

				$template_apply_if_archive_among_cpt = oxy_get_post_meta( $post->ID, 'ct_template_apply_if_archive_among_cpt', true );
				$template_archive_post_types 	= oxy_get_post_meta( $post->ID, 'ct_template_archive_post_types', true );
				if ( !$template_archive_post_types ) {
					$template_archive_post_types = array();
				}
				
				$template_apply_if_archive_among_authors = oxy_get_post_meta( $post->ID, 'ct_template_apply_if_archive_among_authors', true );
				$template_authors_archives 	   = oxy_get_post_meta( $post->ID, 'ct_template_authors_archives', true );
				if ( !$template_authors_archives ) {
					$template_authors_archives = array();
				}

				// index
				$template_index 		= oxy_get_post_meta( $post->ID, 'ct_template_index', true );

				// front page
				$template_front_page 	= oxy_get_post_meta( $post->ID, 'ct_template_front_page', true );

				// blog posts
				$template_blog_posts 	= oxy_get_post_meta( $post->ID, 'ct_template_blog_posts', true );

				// date archive
				$template_date_archive 	= oxy_get_post_meta( $post->ID, 'ct_template_date_archive', true );

				// search result
				$template_search_page 	= oxy_get_post_meta( $post->ID, 'ct_template_search_page', true );

				// Inner Content
				$template_inner_content 		= oxy_get_post_meta( $post->ID, 'ct_template_inner_content', true );
				
				// 404 page
				$template_404_page 		= oxy_get_post_meta( $post->ID, 'ct_template_404_page', true );
...
function ct_view_save_meta_box( $post_id ) {
	...
	if (isset($_POST['ct_use_inner_content'])) {
		...
		oxy_update_post_meta( $post_id, 'ct_use_inner_content', $ct_use_inner_content);
	}
	...
	if (isset($_POST['ct_parent_template'])) {
		...
		if($ct_parent_template == '0' || intval($ct_parent_template) == $post_id) {
			delete_post_meta( $post_id, '_ct_parent_template');
		}
		else {
			oxy_update_post_meta( $post_id, 'ct_parent_template', $ct_parent_template);
		}
	}
	...
	if(isset($_POST['ct_other_template'])) {
		...
		if (is_numeric($ct_other_template)) {
			if ( $ct_other_template !== 0 )
				oxy_update_post_meta( $post_id, 'ct_other_template', $ct_other_template);
			else
				delete_post_meta(  $post_id, '_ct_other_template' );
		}
	} else {
		delete_post_meta(  $post_id, '_ct_other_template' );
	}
...
// template type
	...
	if ( $template_archive_post_types_all ) {
		oxy_update_post_meta( $post_id, 'ct_template_archive_post_types_all', $template_archive_post_types_all );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_archive_post_types_all', "");
	}

	// categories
	oxy_update_post_meta( $post_id, 'ct_template_categories', $template_categories );
	if ( $template_categories_all ) {
		oxy_update_post_meta( $post_id, 'ct_template_categories_all', $template_categories_all );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_categories_all', "");
	}
	
	// tags
	oxy_update_post_meta( $post_id, 'ct_template_tags', $template_tags );
	if ( $template_tags_all ) {
		oxy_update_post_meta( $post_id, 'ct_template_tags_all', $template_tags_all );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_tags_all', "");
	}
	
	// custom taxonomy
	oxy_update_post_meta( $post_id, 'ct_template_custom_taxonomies', $template_custom_taxonomies );
	if ( $template_custom_taxonomies_all ) {
		oxy_update_post_meta( $post_id, 'ct_template_custom_taxonomies_all', $template_custom_taxonomies_all );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_custom_taxonomies_all', "" );
	}

	// authors archives
	
	if ( $template_authors_archives_all ) {
		oxy_update_post_meta( $post_id, 'ct_template_authors_archives_all', $template_authors_archives_all );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_authors_archives_all', "" );
	}

	// index
	oxy_update_post_meta( $post_id, 'ct_template_index', $template_index );

	// front page
	oxy_update_post_meta( $post_id, 'ct_template_front_page', $template_front_page );

	// blog posts
	oxy_update_post_meta( $post_id, 'ct_template_blog_posts', $template_blog_posts );

	// date archive
	oxy_update_post_meta( $post_id, 'ct_template_date_archive', $template_date_archive );

	// search result
	oxy_update_post_meta( $post_id, 'ct_template_search_page', $template_search_page);

	// Inner Content
	oxy_update_post_meta( $post_id, 'ct_template_inner_content', $template_inner_content);

	// 404 page
	oxy_update_post_meta( $post_id, 'ct_template_404_page', $template_404_page);


	if ( $template_all_archives ) {
		oxy_update_post_meta( $post_id, 'ct_template_all_archives', $template_all_archives );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_all_archives', "" );
	}

	oxy_update_post_meta( $post_id, 'ct_template_archive_among_taxonomies', $template_archive_among_taxonomies );
	if ( $template_apply_if_archive_among_taxonomies ) {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_taxonomies', $template_apply_if_archive_among_taxonomies );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_taxonomies', "" );
	}

	oxy_update_post_meta( $post_id, 'ct_template_archive_post_types', $template_archive_post_types );
	if ( $template_apply_if_archive_among_cpt ) {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_cpt', $template_apply_if_archive_among_cpt );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_cpt', "" );
	}

	oxy_update_post_meta( $post_id, 'ct_template_authors_archives', $template_authors_archives );
	if ( $template_apply_if_archive_among_authors ) {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_authors', $template_apply_if_archive_among_authors );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_archive_among_authors', "" );
	}
	...
	oxy_update_post_meta( $post_id, 'ct_template_single_all', 			$template_single_all );
	
	// post types
	oxy_update_post_meta( $post_id, 'ct_template_post_types', 			$template_post_types );
	oxy_update_post_meta( $post_id, 'ct_template_exclude_ids', 			$template_exclude_ids );
	oxy_update_post_meta( $post_id, 'ct_template_include_ids', 			$template_include_ids );
	
	oxy_update_post_meta( $post_id, 'ct_template_taxonomies', 			$template_taxonomies );

	// custom taxonomy
	oxy_update_post_meta( $post_id, 'ct_template_taxonomies', $template_taxonomies );
	if ( $use_taxonomies ) {
		oxy_update_post_meta( $post_id, 'ct_use_template_taxonomies', $use_taxonomies );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_use_template_taxonomies', "" );
	}


	oxy_update_post_meta( $post_id, 'ct_template_post_of_parents', $template_post_of_parents );
	if ( $template_apply_if_post_of_parents ) {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_post_of_parents', $template_apply_if_post_of_parents );
	}
	else {
		oxy_update_post_meta( $post_id, 'ct_template_apply_if_post_of_parents', "" );
	}


	oxy_update_post_meta( $post_id, 'ct_template_order', $template_order );
	...
	if (isset($_POST["ct_custom_view_on_create_copy"]) && $_POST["ct_custom_view_on_create_copy"] == "true") {
		...
		if($other_template > 0) {
			$shortcodes = oxy_get_post_meta( $other_template, "ct_builder_shortcodes", true );
			$json = oxy_get_post_meta( $other_template, "ct_builder_json", true );
		}
		else {
		...
		oxy_update_post_meta( $post_id, 'ct_builder_json', $json);

		// reset the page settings to use custom view
		oxy_update_post_meta( $post_id, 'ct_render_post_using', 'custom_template' );
		delete_post_meta(  $post_id, '_ct_other_template' );
...
function ct_view_order_save_meta_box( $post_id ) {
	...
	if ( isset( $_POST["ct_template_order"] ) ) {
		oxy_update_post_meta( $post_id, 'ct_template_order', intval($_POST["ct_template_order"]) );
	};
}
...
function ct_views_order_value( $column, $post_id ) {
    switch ( $column ) {
        ...
        case 'ct_view_order' :
        	$template_order = oxy_get_post_meta( $post_id, 'ct_template_order', true );
...
function ct_views_order_sort( $query ) {
    ...
    if( 'ct_view_order' == $orderby ) {
        $query->set('meta_key','_ct_template_order');
    ...
    switch ( $column ) {
        ...
        case 'ct_post_type' :
            ...
            if ( $template_type == "single_post" ) {
            	$post_types = oxy_get_post_meta( $post_id, 'ct_template_post_types', true );
            }

            if ( $template_type == "archive" ) {
            	$post_types = oxy_get_post_meta( $post_id, 'ct_template_archive_post_types', true );
            }
...
function ct_views_taxonomies_value( $column, $post_id ) {
    switch ( $column ) {
        ...
        case 'ct_post_taxonomies' :
        	...
        	$template_type = sanitize_text_field(oxy_get_post_meta( $post_id, 'ct_template_type', true ));

            if ( $template_type == "archive" ) {
            	
            	$categories 		= oxy_get_post_meta( $post_id, 'ct_template_categories', true );
            	$categories 		= is_array($categories) ? array_map('sanitize_text_field', $categories): array();


				$tags 				= oxy_get_post_meta( $post_id, 'ct_template_tags', true );
				$tags = is_array($tags) ? array_map('sanitize_text_field', $tags): array();

				$custom_taxonomies 	= oxy_get_post_meta( $post_id, 'ct_template_custom_taxonomies', true );
				$custom_taxonomies 	= is_array($custom_taxonomies) ? array_map('sanitize_text_field', $custom_taxonomies): array();

				$authors_archives 	= oxy_get_post_meta( $post_id, 'ct_template_authors_archives', true );
				$authors_archives 	= is_array($authors_archives) ? array_map('sanitize_text_field', $authors_archives): array();

				$categories_all 		= sanitize_text_field(oxy_get_post_meta( $post_id, 'ct_template_categories_all', true ));
				$tags_all 				= sanitize_text_field(oxy_get_post_meta( $post_id, 'ct_template_tags_all', true ));
				$custom_taxonomies_all 	= sanitize_text_field(oxy_get_post_meta( $post_id, 'ct_template_custom_taxonomies_all', true ));
				$authors_archives_all 	= sanitize_text_field(oxy_get_post_meta( $post_id, 'ct_template_authors_archives_all', true ));
            }
